...
class TestSectionsParser:
    ...
    def test_document_with_only_empty_lines(self):
        doc = "\n\n\n"
        parser = SectionsParser(doc)
        sections = list(parser)
        assert len(sections) == 0

    def test_sections_with_page_breaks(self):
        doc = "Section 1\nLine 2 [Page 1]\nSection 2\nLine 2 [Page 2]"
        parser = SectionsParser(doc)
        sections = list(parser)
        assert len(sections) == 2
        assert sections[0].lines[2].text.endswith("[Page 1]")
        assert sections[1].lines[2].text.endswith("[Page 2]")
        assert sections[0].page == 1
        assert sections[1].page == 2

    def test_sections_with_multiple_page_breaks(self):
        doc = "Section 1\nLine 2 [Page 1]\nSection 2\nLine 2 [Page 2]\nSection 3\nLine 2 [Page 3]"
        parser = SectionsParser(doc)
        sections = list(parser)
    ...
    def test_section_with_empty_lines_within(self):
        doc = "Section 1\nLine 2\n\nLine 3\nLine 4"
        parser = SectionsParser(doc)
        sections = list(parser)
        assert len(sections) == 2
    ...
    def test_section_with_indentation(self):
        ...
        sections = list(parser)
        assert len(sections) == 1
        assert sections[0].lines[2].indentation == 2
    ...
    def test_section_with_special_characters(self):
        ...
        sections = list(parser)
        assert len(sections) == 1
        assert "@#$%^&*" in sections[0].lines[2].text
    ...
    def test_section_with_numbers(self):
        ...
        sections = list(parser)
        assert len(sections) == 1
        assert "12345" in sections[0].lines[2].text
...
Line with 12345 [Page 2]"""
        ...
        assert sections[0].lines[3].text.endswith("[Page 1]")
        assert sections[0].page == 1

        assert sections[1].lines[3].indentation == 2
        assert sections[1].page == 2

        assert "@#$%^&*" in sections[2].lines[7].text
        assert sections[2].lines[8].text.endswith("[Page 2]")
