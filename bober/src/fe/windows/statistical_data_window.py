...
class StatisticalDataWindow(BaseWindow):
    ...
    def create_widgets(self):
        label = ttk.Label(
            self.main_frame,
            text="Select a statistic type to display:",
            font=("Arial", 12),
        )
        ...
        word_button = ttk.Button(
            button_frame,
            text="Words",
            command=lambda: self.display_stats(
                self.stats_manager.get_word_stats()
            ),
        )
        word_button.pack(side=tk.LEFT, padx=5)

        char_button = ttk.Button(
            button_frame,
            text="Word Characters",
            command=lambda: self.display_stats(
                self.stats_manager.get_word_char_stats()
            ),
        )
        ...
        custom_button = ttk.Button(
            button_frame,
            text="Non white characters",
            command=lambda: self.display_stats(
                self.stats_manager.get_non_white_char_stats()
            ),
        )
        ...
        custom_button = ttk.Button(
            ...
            text="All characters",
            command=lambda: self.display_stats(
                self.stats_manager.get_all_char_stats()
            ),
        )
        custom_button.pack(side=tk.LEFT, padx=5)
